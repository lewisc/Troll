...
struct
  ...
  fun stringIVal (Interpreter.VAL l) = stringVal l
    ...
    | stringIVal (Interpreter.PAIR (v,w)) =
  ...
  fun printDist1 [] pad = ()
    | printDist1 ((a,p)::l) pad =
	  ...
	  val s3 = padrealtostring p
          val pgeq = case (!col2) of
                     "ge" => p + List.foldr (op +) 0.0 (List.map (#2) l)
                   | "le" => !pmax - List.foldr (op +) 0.0 (List.map (#2) l)
                   | "gt" => List.foldr (op +) 0.0 (List.map (#2) l)
                   | _    => !pmax - p - List.foldr (op +) 0.0 (List.map (#2) l)
  ...
  fun printDist l =
	  ...
	  val maxLen = List.foldr Int.max 0
		        (List.map (fn (Interpreter.VAL a,_) =>
                    ...
                    | (pair,_) => String.size (stringIVal pair))
				  l)
	  ...
	  val l1 = if !percent
	           then (pmax := 100.0; List.map (fn (a,p)=>(a,100.0*p)) l)
		   else l
        in
	  (if !graph=0.0
		     ...
		     NONE =>
		       ...
		       then (case Real.fromString
                             ...
                             | SOME s =>
                               (graph := s;
			        run0 args filename n defs))
		       else if arg = "ge" orelse  arg = "le" orelse
		               arg = "gt" orelse  arg = "lt"
		       then (col2 := arg;
			     run0 args filename n defs)
